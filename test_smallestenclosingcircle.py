...
class SmallestEnclosingCircleTest(unittest.TestCase):
	...
	def test_matching_naive_algorithm(self):
		...
		for _ in range(TRIALS):
			...
			actual = smallestcircumscribingcircle.make_circle(points)
	...
	def test_translation(self):
		...
		for _ in range(TRIALS):
			...
			reference = smallestcircumscribingcircle.make_circle(points)
			...
			for _ in range(CHECKS):
				...
				translated = smallestcircumscribingcircle.make_circle(newpoints)
	...
	def test_scaling(self):
		...
		for _ in range(TRIALS):
			...
			for _ in range(CHECKS):
				...
				scaled = smallestcircumscribingcircle.make_circle(newpoints)
...
def _smallest_enclosing_circle_naive(points):
	...
	for i in range(len(points)):
		...
		for j in range(i + 1, len(points)):
			q = points[j]
			c = smallestcircumscribingcircle.make_diameter(p, q)
			if (result is None or c[2] < result[2]) and \
					all(smallestcircumscribingcircle.is_in_circle(c, r) for r in points):
				result = c
	...
	for i in range(len(points)):
		...
		for j in range(i + 1, len(points)):
			...
			for k in range(j + 1, len(points)):
				r = points[k]
				c = smallestcircumscribingcircle.make_circumcircle(p, q, r)
				if c is not None and (result is None or c[2] < result[2]) and \
						all(smallestcircumscribingcircle.is_in_circle(c, s) for s in points):
					result = c
